...
### 阶段8：logid完整移植 + CC4提前触发DPT（已完成 ✅）

**目标**：对齐ST参考工程的所有logid + 优化编码器时序

**8A. logid完整列表（对齐ST参考工程）：**
...
| 11 | 编码器raw + 位置详细 | ✅ 更新 |
| 30 | DQ轴电压 | ✅ |
| 40 | 电流PI（7列含Iq_filt） | ✅ 更新 |
| 50 | 速度环（5列含output） | ✅ 更新 |
| 60 | CCR占空比 | ✅ 新增 |
| 70 | CCR + 三相电流 | ✅ 新增 |
| 90 | 三相电流raw | ✅ |
| 100 | 位置环 | ✅ |
| 110 | ISR分段耗时（tot/read/enc/pos/vel/cur） | ✅ 更新 |
| 120 | 开环测试状态 | ✅ |
| 130 | DPT编码器统计 | ✅ |
| 140 | CC4/Enc/ADC相对时序 | ✅ 重写 |
| 150 | ADC采样率+电流raw | ✅ |
| 151 | VDC+温度监控 | ✅ |
| 160 | 写Flash | ✅ 新增 |
| 161 | 擦Flash | ✅ 新增 |
| 162 | FlashData dump | ✅ 新增 |
| 163 | 清除故障标志 | ✅ 新增 |
| 164 | 设置位置零点 | ✅ 新增 |
| 165 | 查询故障标志 | ✅ |

**8B. case 140 CC4/Enc/ADC相对时序（对齐ST）：**

新增ISR时间戳记录：
- ADC ISR入口/出口 → `g_adc_isr_in_cycles` / `g_adc_isr_out_cycles`
- TMR1 CC4入口/出口 → `g_tim1_cc4_cycles` / `g_tim1_cc4_exit_cycles`
- DPT IDLE完成时刻 → `g_tim1_enc_done_cycles`

打印格式（T0=ADC_in为基准，DWT @192MHz）：
```
T0=ADC_in | ADC_out=+6us CC4_in=+51us CC4_out=+52us Enc_done=+95us
```

负值修正：CC4/Enc可能在上一周期触发，负值自动+100us换算到当前周期。

**8C. CC4提前触发DPT编码器：**

**原架构（阶段7）：**
- DPT_AsyncRequest() 在 ADC ISR 里调用
- 编码器数据在 ADC ISR 中段到达（T0+44us）
- Enc_done 与 FOC 调度同周期
...
**优势：**
- FOC调度开始时编码器数据已就绪（零延迟）
- ADC ISR耗时减少（不含DPT触发开销）
- 时序可控（通过调整CC4比较值优化Enc_done时刻）
...
T0=ADC_in | ADC_out=+6us CC4_in=+51us CC4_out=+52us Enc_done=+95us
```
- CC4在上数阶段T0+51us触发（下数阶段被跳过）
- DPT round-trip 44us → Enc_done≈95us
- 编码器数据在下一次ADC前5us完成
