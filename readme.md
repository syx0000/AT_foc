...
2026.09.04.11 故障与辨识诊断完善：统一电机故障支持首错锁存、同事件多故障合并及最近5次RAM历史记录，串口fault可查询并返回明确清除拒绝原因；
              软件过流和TMR1硬件Break改为中断内立即关断、主循环安全投递并打印，Break恢复前防止重复中断，CPU异常进入停机死循环前关闭功率级；
              自动辨识采用error失败阶段与detail底层状态两级错误，细分ADC零偏、Rs、Ld/Lq、Hall扫描、Hall补偿、PI和Id/Iq验证原因；
              所有完整及单项辨识CLI在完成、执行失败或因非READY状态被拒绝时均强制回显结果，消除命令静默返回；
              新增Hall转子补偿辨识及测试，完善自动辨识与重构设计文档，并新增全工程错误码、CLI、Flash、OTA及CPU异常对照手册；
              默认关闭周期诊断打印，保留按需查询接口；Keil全量构建通过，0 Error、0 Warning，相关保护与辨识仍需目标硬件验证。

2026.09.04.10 Hall转子角度补偿自动辨识：新增无硬件依赖的定点角度辨识算法，使用正Iq低功率闭环稳定段平均Vd/Vq计算`atan2(-Vd,Vq)`修正量；
              `calibrate hall offset`由占位错误改为执行受限测量，临时应用候选Hall表和PI，退出后恢复原活动参数并仅更新候选补偿，仍需diff/accept/save；
              完整commissioning流程在Hall标定和电流PI计算后加入补偿辨识，并再次执行Id/Iq低功率验证，单次修正限制60°电角且要求平均Vq不低于500 mV；
              新增正常、零误差、负修正、低信号及超限测试向量，工程加入新内核文件；相关变更源文件已通过ARMCC编译，算法断言和实物辨识待验证。

2026.09.04.9  电机参数Flash双槽保存：将末尾4 KB划分为两个独立2 KB擦除槽，OTA暂存上限由254 KB调整为252 KB，仍覆盖238 KB最大App镜像；
              参数记录包含magic、格式版本、长度、递增序号、完整参数、CRC32和最后写入的提交标记，启动时选择最新有效槽；
              保存只擦除非活动槽，数据与CRC回读通过后才写提交标记，写入中断或掉电时保留旧槽，并支持序号回绕比较；
              启动自动加载有效参数，正式电流环和速度环读取活动PI；串口新增storage/load/defaults/save命令，安全切换后同步重载Hall解码器及控制器；
              同步更新App与Bootloader存储边界并增加双槽异常模拟源码；55个工程及测试源文件通过ARMCC编译，12个Bootloader源文件通过ARMClang编译，Flash掉电测试待上板验证。

2026.09.04.8  电机辨识编排与单项命令：新增统一辨识管理器，串联ADC零偏、Rs、Ld/Lq、双向Hall扫描、电流PI计算以及Id/Iq接管验证；
              新增基于候选Rs/Ld/Lq和带宽的Q15电流PI纯算法设计器，并增加边界及标称参数测试；
              候选Hall和电流PI在低功率验证期间临时生效，成功或失败均恢复原活动参数，避免未确认参数直接影响正式运行；
              串口新增完整流程、状态、中止及各辨识/测试单项命令，单项结束自动打印结果，统一复用diff/accept/discard；
              Keil工程加入新增模块并完成53个工程及测试源文件ARMCC编译。当前为阻塞式编排，Hall转子补偿、实物验证和Flash保存仍待后续完成。

2026.09.04.7  Hall自动标定纯算法：foc_kernel新增独立标定上下文，分正向扫描、反向验证和结果输出三个阶段，不访问GPIO、PWM、日志及运行时参数；
              正向扫描自动发现六状态唯一后继并累计进入边界角，反向扫描校验正向逆序并将同一物理边界的角度进行交叉验证；
              使用参考角附近的有符号展开平均正确处理0/65535跨界，输出Hall正向表、六边界角、扇区宽度、正反样本数及最大离散度；
              增加非法状态、重复、跨状态、多后继、缺状态、事件超时、扇区异常、正反不一致等明确错误码，失败结果保持invalid；
              新增36种相线/Hall排列组合及异常路径测试，Keil工程加入motor_hall_calibration.c并完成47个工程源文件和3个测试源文件ARMCC编译。
