...
运行时参数管理、Hall端口解耦、运行时解码、双向Hall角度估算、逻辑方向以及Hall自动标定纯算法已经完成。完整`motor_commissioning`状态机、串口单项触发、候选结果自动写入和Flash双槽保存尚未实现。工程继续保持默认不开PWM，并在完成实物验证前禁止正式物理反向闭环。
...
Hall顺序和边界角度可以自动采集。转子角度补偿首版使用正Iq低功率闭环稳定段平均电压，根据`atan2(-Vd,Vq)`计算相对当前补偿的修正量；要求Vq达到有效门槛并限制单次最大修正角。结果只写入候选参数，用户仍须检查测量值和`diff`后再决定是否`accept/save`。后续可根据实测需要增加多点搜索最小Vd、最小母线功率或双向交叉验证。

#### 待办：基于速度与系统延迟的动态角度补偿

当前只使用一份正反向共用的静态`hall_rotor_offset_u16`，暂不把动态角度补偿接入正式控制链。先完成静态补偿、正向闭环和反向低功率交叉验证，避免用动态项掩盖Hall边界表、方向映射或静态转子坐标误差。
...
每个点记录平均`Vd/Vq`、`Id/Iq`、母线电流、Hall周期、电流环限压状态，以及由`atan2(-Vd,Vq)`得到的剩余修正角。只有在误差随有符号电角速度近似线性、正反向基本对称时，才认为主要误差来自固定系统延迟，并按下式估算：

```text
动态补偿角 ≈ 有符号电角速度 × 系统总延迟
控制角度 = Hall估算角 + 静态转子补偿 + 动态补偿角
```

决策门槛：最高工作速度下剩余动态误差小于约3°电角时暂不实现；超过5°且随速度近似线性时再实现并启用。若正反向不对称，或误差不随速度线性变化，应先检查Hall标定表、采样/PWM时序、速度估算和机械负载，不增加方向独立的固定offset来掩盖问题。

实现时应预留但默认关闭动态补偿参数，限制最大补偿角，并在Hall失效、速度无效或方向不确定时立即将动态项清零。该功能必须经过正反向、多速度、加减速和交接过程验证后才能默认启用。
...
- `motor_hall_port`已缩减为原始GPIO组合、边沿计数和周期时间戳采集；
- `motor_hall_decoder`已放入`foc_kernel`，初始化时复制活动参数中的Hall正向表，不依赖AT32寄存器、BSP或应用层；
- 合法性检查除唯一六步闭环外，还要求相邻状态只变化一个Hall位；
- 方向、异常计数和最近6段边沿滑动测速已迁移到解码器，并使用无符号周期差支持DWT自然回绕；
- 主循环通过序列锁读取一致快照；Hall中断不执行日志、动态内存或额外外设操作，测速沿用原有64位除法；
- 角度观察器、角度估算器、速度反馈和诊断输出已迁移到解码快照接口；
- 已新增纯算法测试源文件，覆盖正反方向、六种HA/HB/HC位排列、重复状态、非法电平、跨状态、方向切换和时间戳回绕。当前环境没有主机C编译器，测试源已通过ARMCC编译，运行断言待后续接入主机测试执行器。
...
实施状态（2026.09.04）：代码完成，反向实物验证待执行。

- Hall角度边界与相位步长计算已下沉为无硬件依赖的`motor_hall_angle_math`，正向和反向在同一物理边界得到一致校正角；
- `motor_hall_angle_estimator`已支持正负相位步长及U16自然回绕，并在快照中输出物理方向；
- `motor_speed_feedback`使用活动参数中的极对数和`direction_inverted`，输出统一的逻辑有符号机械转速；
- 速度PI输入使用逻辑有符号rpm，输出逻辑Iq后通过同一方向变换生成物理Iq指令；
- 超速和堵转判断已改为双向绝对值判断，目标方向与反馈方向必须一致，运行中禁止目标跨零反转；
- 串口已增加`motor direction [normal/reverse]`，方向修改要求READY、PWM关闭且不存在其他候选差异；
- `MOTOR_REVERSE_CONTROL_VERIFIED`当前为0，允许配置逻辑方向，但所有会形成物理反向的正式速度闭环指令均被拒绝；
- 新增方向变换、正反边界、相位步长及角度回绕测试源，并与全部46个工程源文件通过ARMCC编译。主机测试执行器仍待接入。
...
实施状态（2026.09.04）：纯算法与测试源完成，尚未接入驱动电机的标定状态机。

- 新增可同时创建多个实例的`motor_hall_calibration_t`，算法不持有全局硬件状态；
- 正向扫描从状态跳变自动建立六状态唯一后继表，并按进入后的状态累计物理边界角；
- 反向扫描要求每个跳变严格等于正向逆序，并按边沿前状态累计同一物理边界角；
- 边界角使用相对首样本的有符号展开平均，支持样本跨越0/65535；
- 结果包含正向表、边界角、正向扇区宽度、正反样本数、最大边沿离散和最大正反误差；
- 配置可限制最少样本、扇区宽度、单边重复性、正反一致性和最大事件间隔；
- 任何非法状态、重复状态、非单bit跳变、多后继、缺状态、异常扇区、超差或反向不一致都会锁存FAULT，`result.valid`不会成立；
- 测试源覆盖6种相序模型乘6种Hall位排列、循环起点/方向、角度跨零、DWT回绕及各类失败路径；当前环境无主机C执行器，3个测试源均已通过ARMCC编译，断言运行待测试执行器接入。
...
实施状态（2026.09.04）：阶段5首版代码完成，待上板验证。

- 新增`motor_commissioning`统一管理完整流程和单项任务，同一时刻只允许一个任务运行；完整流程依次执行ADC零偏、Rs、Ld/Lq、双向Hall扫描、电流PI计算、Id验证和Iq/Hall接管验证。
- Rs、Ld/Lq、Hall表和电流PI只写入候选参数；低功率测试前临时应用候选Hall与PI，退出路径无条件停止PWM并恢复原活动参数，用户仍需通过`diff/accept/discard`确认。
- 新增`motor_current_pi_design`纯算法，按候选Rs、Ld/Lq、带宽和10 kHz采样频率生成Q15增益；原测试接口保持兼容，新增指定PI增益的测试入口用于候选参数验证。
- 串口已加入`motor commissioning start/status/abort`以及ADC零偏、Rs、Ld/Lq、Hall、PI、Id、Iq和接管单项命令；每个单项结束自动打印结果。
- 自动辨识错误采用两级诊断：`error`表示失败阶段，`detail`保留对应模块的原始状态码。当前覆盖ADC零偏、Rs、Ld/Lq、Hall双向扫描、Hall补偿、PI设计、Id/Iq验证和候选参数操作；`status`与失败即时日志同时打印数字码及可读名称。
- 当前编排复用既有阻塞式辨识/测试接口，因此主循环在任务执行期间不能继续解析同一串口的`status/abort`；中止接口可供外部安全路径调用，后续如需在线交互再把各硬件步骤拆成非阻塞子状态。
- `calibrate hall offset`已接入正Iq低功率测量和定点相角求解，自动生成`hall_rotor_offset_u16`候选值；当前仍需完成算法断言运行和实物验证，不自动accept或写Flash。
- 代码已完成53个工程及测试源文件ARMCC编译和Keil工程XML检查；Hall双向扫描、候选参数低功率试运行及故障回滚仍需实物验证，不记录为PASS。
...
实施状态（2026.09.04）：阶段6代码完成，待Flash掉电上板验证。

- 参数区扩展为末尾两个独立2 KB擦除扇区：槽A=`0x0807F000`，槽B=`0x0807F800`；Staging上限同步由254 KB调整为252 KB，仍大于238 KB的App最大尺寸。
- 每槽记录包含magic、格式版本、参数长度、递增序号、完整`motor_parameter_t`、CRC32和提交标记；提交标记最后单独写入。
- 启动扫描两个槽，验证格式、长度、CRC、参数范围和提交标记，使用带回绕处理的序号比较选择最新有效记录；无有效记录时保留编译期默认参数。
- 保存时选择非活动槽，依次执行擦除、写记录主体、回读校验、写提交标记和最终复核；任一步失败均不会破坏原活动槽。
- 串口新增`motor commissioning save`、`motor params load/defaults/storage`；只保存已经生效的活动参数，候选参数不能直接写Flash，运行时操作要求READY且PWM关闭。
- 正式电流环和速度环初始化改为读取活动PI；参数加载、恢复默认值或接受候选值后同步重载Hall解码器及两个控制器；`defaults`只改变RAM，必须显式执行`save`才会持久化。
- 新增双槽模拟源码覆盖空Flash、A/B轮换、未提交新槽和CRC损坏回退；当前环境无主机C测试执行器，仅完成ARMCC编译，断言运行和真实掉电测试待后续执行。
