...
void Init_foc(ControllerStruct* controller) {
    ...
    // 9. ISR 守卫标志：所有结构体/滤波器初始化完成后再放行 PCCE ISR 调度
    g_foc_init_done = 1;
}

/* ISR scheduling guard: PCCE fires at 10kHz from boot; FOC code only runs after Init_foc() */
volatile uint8_t g_foc_init_done = 0;
