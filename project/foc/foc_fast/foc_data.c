...
/* Motor pole pairs (eyou 70: NPP=8, eyou 80: NPP=4)
...
void ElecAngleEstimate(ControllerStruct* controller) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        /* 诊断打印: 看 PWM 真的写到 TMR1 没 / MOE 在不在 / udc 对不对 */
        printf("  vd=%d theta=%u udc=%u brk=0x%08lX\r\n"
               "    CCR(local)=%lu/%lu/%lu  TMR1.cXdt=%lu/%lu/%lu\r\n",
               (int)v_d, (unsigned)theta_open[i], (unsigned)g_udc_volt,
               (unsigned long)TMR1->brk,
               (unsigned long)controller->CCR2,
               (unsigned long)controller->CCR3,
               (unsigned long)controller->CCR4,
               (unsigned long)TMR1->c1dt,
               (unsigned long)TMR1->c2dt,
               (unsigned long)TMR1->c3dt);
