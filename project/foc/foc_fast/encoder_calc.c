...
#include "wk_usart.h"       // Core/Inc/encoder.h - DPT驱动
#include <stdlib.h>
#include <stdio.h>
...
/* ============================================================
 * Encoder angle compensation (calibrated by can_debug)
 * Migrated from cubemx_yxsui/foc/foc_fast/foc_bsp.c
 * ============================================================ */
int16_t g_theta_offset_pos = 0;   /* -12.0° (deg * 10), set by phase calibration */
int16_t g_theta_offset_neg = 0;
int16_t g_theta_comp_pos   = 20;  /* deg * 10, speed-dependent compensation */
int16_t g_theta_comp_neg   = 0;
...
void Encoder_data_Calculate(ControllerStruct* controller, uint16_t hz) {
    ...
    if (!controller->ident_test.enable) {
        ...
        /* 方向相关相位补偿 = 固定偏置 + 速度相关项
         *   theta_comp = offset_counts + dtheta * comp_coef / 10
         *   offset: 方向相关固定误差 (×0.1°), dbg: offsetpos/offsetneg
         *   comp:   速度相关延迟 (×0.1 倍 dtheta), dbg: comppos/compneg
         * 诊断结论: 编码器方向相关相位误差, 正反转不对称 */
        ...
        int32_t off_01deg, coef;
        if (dtheta >= 0) { off_01deg = g_theta_offset_pos; coef = g_theta_comp_pos; }
        else             { off_01deg = g_theta_offset_neg; coef = g_theta_comp_neg; }
        int32_t theta_comp = (off_01deg * 1820) / 100 + (dtheta * coef) / 10;
