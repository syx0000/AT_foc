...
### 7.1 待办：开环到 Hall 电流闭环的无扰交接

当前启动路径保持为：

```text
READY → OPEN_LOOP → CURRENT_CONTROL → SPEED_CONTROL
```

`OPEN_LOOP → CURRENT_CONTROL` 不应仅以 Hall 当前状态有效作为交接条件。后续实现需要增加交接资格判定，并要求下列条件连续满足一段可配置时间：

- Hall 状态合法，连续跳变顺序与目标旋转方向一致；
- 已捕获至少 6～12 个连续合法 Hall 边沿，具体门槛通过上板测试确定；
- Hall 估算速度超过可配置的最低接管速度；
- Hall 插值角与开环电角度的最短圆周误差小于约 15°～20°电角，最终阈值通过实测确定；
- 相电流、母线电压、门极驱动器和硬件刹车状态均正常；
- 避免在 Hall 边沿附近直接切换，防止边沿更新与控制权交接发生在同一个快速周期。

交接阶段增加数十毫秒的角度融合，融合系数 `k` 从 0 平滑递增到 1：

```text
控制角度 = 开环角度 + k × shortest_angle(Hall估算角度 - 开环角度)
k: 0 → 1
```

角度融合必须沿最短圆周方向计算，不能直接对两个 U16 周期角做普通线性平均。融合过程中继续监控 Hall 合法性、角度误差和电流；任一资格条件失效时中止交接并按安全策略退回开环或停机。

同时保留现有电压前馈交接：将开环实际 `Vd/Vq` 投影到 Hall 坐标系，并作为电流 PI 输出初值。后续还需补充以下无扰初始化：

- 电流参考从交接前的实际 `Id/Iq` 或等效转矩指令开始，再斜坡到目标值；
- `CURRENT_CONTROL → SPEED_CONTROL` 时，速度斜坡以当前实测速度为初值；
- 速度 PI 积分项以切换前有效的 `Iq` 指令为初值，避免接管瞬间转矩跌落；
- 为交接资格失败、融合超时和交接后电流突变定义独立故障码及诊断快照。

### 7.2 待办：启动性能与大负载能力演进

近期先在现有电压开环结构上完成可测量、可回退的 Hall 交接，第一版建议参数为：
...
上述数值是首次上板的保守起点，不作为最终产品参数。实际交接点必须同时依据 Hall 实测速度、开环频率、角度误差和相电流确定；开环频率达到门槛但转子实际速度未跟上时禁止交接。

近期实施顺序：

1. 增加交接资格判定、角度融合、失败回退和诊断快照；
2. 完成电流 PI、速度斜坡和速度 PI 的无扰初始化；
3. 记录开环频率、Hall 频率、角度误差、`k`、`Id/Iq`、`Vd/Vq` 和峰值相电流；
4. 空载验证通过后逐级增加负载、电流上限和加速度，不允许一次放开至额定值。

对大负载启动，后续将电压开环升级为 I/F 启动：电角度仍按受控斜坡开环推进，但从启动阶段起即运行 `Id/Iq` 电流闭环并以受限 `Iq` 控制启动转矩。Hall 稳定后只融合角度源，电流控制器持续运行：
...
- 启动 `Iq` 斜坡、最大启动电流和超时限制；
- 转子跟随/失步判定，不允许仅依据开环角频率认为电机已经转动；
- 带载启动失败时降加速度、限次重试或安全停机，不盲目提高电压；
- 母线欠压、过流、堵转、Hall 非法和驱动器故障的快速退出；
- 正反方向分别验证，禁止直接复用未经验证的角度符号和 Hall 顺序。

低速策略需要与启动策略分开：完成 Hall 接管后，即使目标速度低于交接速度也保持闭环，不自动退回开环。若从静止启动且目标低于可靠交接速度，可在机械允许时先升至交接速度、完成闭环后再减速；不允许速度越升的设备，需要增加编码器、磁编码器或专门的 Hall 扇区低速转矩策略。接近零速时不承诺仅靠 Hall 实现平滑、精确的转矩和速度控制。

第一阶段建议验收指标：

- 交接峰值电流不超过稳定启动电流的 1.5 倍；
- 交接转速跌落不超过交接前速度的 10%；
- 交接后 300 ms 内恢复稳定；
- 不发生反转、非法 Hall 跳变、持续 PI 饱和或硬件保护动作。

调试稳定后再将目标收紧至转速跌落小于 5%、恢复时间小于 100～200 ms。启动总延迟、速度响应和允许负载必须形成实测曲线，不以单一固定参数宣称性能。

以上 7.1 和 7.2 目前均仅作为设计待办，尚未在代码和上板测试中实现。
...
## 16. 当前实现与实测基线（2026.09.04.9）

当前总体处于“核心功能基本齐备、系统上板验证尚未闭环”的阶段。软件主体已经覆盖采样、Hall反馈、FOC电流环、速度环、自动辨识编排和参数持久化；现阶段的主要工作应从继续增加模块转向整机集成、故障注入和实物性能验证。
...
- Hall默认正向顺序为`1→5→4→6→2→3`，六状态边沿角度已有实测标定数据并支持边沿间插值。
- 阻塞辨识曾实测`Rs=273 mΩ、Ld=225 µH、Lq=206 µH`。
- 固定角度200 Hz电流PI测试曾实测`Id=2 A`指令下平均1991 mA，`Iq`平均-12 mA。
- DRV8353唤醒、nFAULT/BIF检查、PWM紧急关闭、中断频率和DWT性能监控已有上板记录。

### 16.2 已完成代码和工程集成

- 已实现默认、活动、候选三层运行时电机参数，支持字段校验、`diff/accept/discard`和临时试运行后恢复。
- Hall端口、运行时解码器、正反向角度估算和逻辑方向已解耦；正式物理反向闭环仍由安全开关禁止。
- Hall自动标定纯算法支持正向扫描、反向验证、边界角统计和异常检测。
- `motor_commissioning`已串联ADC零偏、Rs、Ld/Lq、Hall扫描、电流PI计算及Id/Iq/接管测试，并提供完整流程和单项CLI命令。
- Flash参数区已实现A/B双槽、递增序号、CRC32、提交标记、最新有效槽选择及旧槽回退；启动时优先加载有效Flash参数，无有效槽时使用编译默认值。
- App与Bootloader已同步更新参数区和OTA暂存区边界；相关工程源文件已完成ARMCC/ARMClang编译检查。

### 16.3 尚未闭环的验证

- 最近架构调整后的10 kHz电流环、速度环和全部安全关闭路径需要完整上板回归。
- 速度闭环动态响应、堵转、超速、方向保护及物理反向控制尚未完成实物验证。
- Hall双向自动扫描、36种接线组合、候选参数临时应用和失败恢复尚未形成实物PASS记录。
- Flash A/B槽轮换、写入中断、CRC损坏、序号回绕和真实掉电恢复尚未上板验证。
- 自动辨识当前为阻塞式编排，执行期间主循环不能继续处理同一串口的`status/abort`；后续如需在线交互，需要拆成非阻塞子状态机。
- 开环到Hall电流闭环的连续资格判断、最短圆周角度融合、速度PI无扰预置和I/F大负载启动仍属于7.1、7.2节待办。
- Hall转子角度补偿自动辨识代码已完成并接入候选参数流程，算法断言和实物辨识尚待验证；精确零速/超低速控制、位置闭环、CAN业务协议和产品级温升/额定能力验证尚未完成。

### 16.4 当前阶段结论

当前代码不能仅凭“编译通过”视为产品功能完成。进入下一阶段前，应优先完成基础安全回归、参数Flash掉电验证、辨识单项验证、完整辨识流程验证以及开环到电流环再到速度环的动态波形测试；验证记录达到通过标准后，才逐级开放反向、更高电流、更高速度和更高调制度。
...
- 正式启动流程为DRV安全检查、ADC零偏校准、进入READY并等待命令；上电不再自动执行参数辨识或电机试转。
- `motor_control`统一维护STARTUP、READY、OPEN_LOOP、CURRENT_CONTROL、SPEED_CONTROL和FAULT状态，通信层不得绕过状态机操作PWM。
- USART1使用DMA1 Channel4接收和IDLE分帧，ISR仅提交数据；CLI解析与命令执行位于主循环。
- USART1继续使用轮询`printf`输出日志，关闭未配置的TX DMA；USART3/RS485正式二进制协议后续单独实现。
- 当前CLI开放状态/故障、日志等级、安全停止、开环、电流环、速度环、运行时参数、自动辨识及Flash参数存储命令，调试电流指令限制为±2 A。
- 电机极对数由`MOTOR_POLE_PAIRS=4`统一配置，机械转速按`rpm=电频率mHz×60/(极对数×1000)`换算。
- 200 Hz电流PI采用毫安输入、毫伏输出和条件积分抗饱和；固定角度`Id=2 A`测试平均值为1991 mA，Iq平均值为-12 mA。
- 当前正式启动只执行DRV安全检查和ADC零偏校准，成功后进入READY等待命令；不会在上电时自动辨识或试转。
