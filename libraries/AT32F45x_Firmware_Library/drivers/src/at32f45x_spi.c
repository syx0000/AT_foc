...
/**
  ...
  *         SPI1, SPI2, SPI3, SPI4, I2SF5, I2S2EXT, I2S3EXT
  * @retval none
  */
void spi_i2s_reset(spi_type *spi_x)
{
  ...
  else if(spi_x == I2S2EXT)
  {
    crm_periph_reset(CRM_I2S2EXT_PERIPH_RESET, TRUE);
    crm_periph_reset(CRM_I2S2EXT_PERIPH_RESET, FALSE);
  }
  else if(spi_x == I2S3EXT)
  {
    crm_periph_reset(CRM_I2S3EXT_PERIPH_RESET, TRUE);
    crm_periph_reset(CRM_I2S3EXT_PERIPH_RESET, FALSE);
  }
}
...
void i2s_init(spi_type* spi_x, i2s_init_type* i2s_init_struct)
{
  ...
  else
  {
    if(spi_x == I2SF5)
    {
      ...
      {
        i2s_sclk_index = (uint32_t)48000000;
      }
